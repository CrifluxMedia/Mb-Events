...
const ForgotPassword = () => {
  ...
  const url =
    "https://mbevents-server-mercy.onrender.com/api/v1/forgot-password";

  const onSubmit = async (data) => {
    try {
      const result = await axios.post(url, data);
      if (result.status === 200) {
        toast.success("Password rest link sent to your mail", {
          position: "top-center",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="vh-100 d-flex justify-content-center align-items-center reset-container">
      <form onSubmit={handleSubmit(onSubmit)} className="p-2">
        ...
        <ActionBtn
          width={"100%"}
          content={isSubmitting ? "Reset Password..." : "Reset Password"}
          type="submit"
